...
response = agent.invoke(
    {
        "messages": [
            {"role": "user", "content": "What are the current limitations of Agent capabilities?"}
        ]
    }
)
...
class ReRankOutput(BaseModel):
    indices: List[int] = Field(
        description="List of indices of recalled text fragments after reranking"
    )


# Return at most limit text chunks
def get_relevant_texts(query: str, dense_texts: list, sparse_texts: list, limit: int = 3):
